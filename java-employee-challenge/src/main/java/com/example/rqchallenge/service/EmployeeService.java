...
public class EmployeeService implements IEmployeeService {
	...
	public Employee getEmployeeById(String id) throws EmployeeAPIException {
		try {
			...
			if(response.getData() == null) {
				logger.info("Could not fetch employees from data source");
				throw new EmployeeAPIException("Could not get employee with id: " + id + " from data source ", HttpStatus.NOT_FOUND);
			}
			return response.getData();
		} catch (TooManyRequests e) {
			logger.info("Too many requests", e.getMessage(), e);
	...
	public Employee createEmployee(Map<String, Object> employeeInput) throws EmployeeAPIException {
		try {
			...
			throw new EmployeeAPIException("Could not create an employee in data source ", HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public String deleteEmployeeById(String id) throws EmployeeAPIException {
		try {
			...
			throw new EmployeeAPIException("Could not delete an employee with id " + id + " from data source ", HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public Integer getHighestSalaryOfEmployee() throws EmployeeAPIException {
		try {
			...
			throw new EmployeeAPIException("Could not fetch highest employee salary from data source due to " + HttpStatus.INTERNAL_SERVER_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public List<String> getTopTenHighestEarningEmployeeNames() throws EmployeeAPIException {
		try {
			...
			logger.info("Could not fetch employees from data source", e.getMessage(), e);
			throw new EmployeeAPIException("Could not fetch top ten highest earning employees from data source due to " + HttpStatus.INTERNAL_SERVER_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
